...
Class Master extends DBConnection {
	...
	function save_offense(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Rule code already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Rules successfully saved.");
			else
				$this->settings->set_flashdata('success',"Rules successfully updated.");
		}else{
			$resp['status'] = 'failed';
	...
	function delete_offense(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Rules successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_driver(){
		...
		if($del){
			...
			if(is_file(base_app.$image_path))
				unlink((base_app.$image_path));
			$this->settings->set_flashdata('success',"Officer Info successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_offense_record(){
		...
		if($chk > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Offense Register No. already exist in the database. Please review and try again.";
			return json_encode($resp);
			exit;
		}
